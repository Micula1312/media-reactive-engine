...
APP_DATA_ROOT = (Path(os.environ.get("LOCALAPPDATA", Path.home())) / APP_NAME) if os.name == "nt" else (Path.home() / ".media-reactive-engine")
...
def load_config():
    ...
    try:
        if CONFIG_FILE.exists(): data = json.loads(CONFIG_FILE.read_text(encoding="utf-8"))
...
def choose_folder(initial_dir=None, title="Choose Media Folder"):
    ...
    if os.name == "nt":
        ...
        try:
            result=subprocess.run(["powershell.exe","-NoProfile","-STA","-Command",script],capture_output=True,text=True,timeout=120,check=False); selected=result.stdout.strip(); return Path(selected).expanduser().resolve() if selected else None
        except (OSError,subprocess.SubprocessError): return None
    try:
        import tkinter as tk
        from tkinter import filedialog
        root=tk.Tk(); root.withdraw(); root.attributes("-topmost",True); selected=filedialog.askdirectory(initialdir=initial,title=title); root.destroy(); return Path(selected).expanduser().resolve() if selected else None
...
@app.get("/api/config")
def config(): return jsonify({"media_root":str(MEDIA_ROOT),"visual_root":str(VISUAL_ROOT),"audio_root":str(AUDIO_ROOT),"app_data_root":str(APP_DATA_ROOT),"exists":MEDIA_ROOT.exists(),"saved":CONFIG_FILE.exists()})
...
def select_media_root():
    ...
    MEDIA_ROOT=selected; VISUAL_ROOT=MEDIA_ROOT/"video"; AUDIO_ROOT=MEDIA_ROOT/"audio"; VISUAL_ROOT.mkdir(parents=True,exist_ok=True); AUDIO_ROOT.mkdir(parents=True,exist_ok=True); save_config(); return jsonify({"cancelled":False,"media_root":str(MEDIA_ROOT),"visual_root":str(VISUAL_ROOT),"audio_root":str(AUDIO_ROOT)})
...
def update_visual_state():
    merge_visual_state(request.get_json(silent=True) or {}); return jsonify(VISUAL_STATE)
...
if __name__=="__main__":
    print(f"MEDIA COLLECTION: {MEDIA_ROOT}"); print(f"VIDEO LIBRARY:    {VISUAL_ROOT}"); print(f"AUDIO LIBRARY:    {AUDIO_ROOT}"); print("CONSOLE: http://127.0.0.1:5000/console"); app.run(host="127.0.0.1",port=5000,debug=True,use_debugger=False,use_reloader=True)
