...
app = Flask(__name__)
APP_NAME = "magic-mic"
LEGACY_APP_NAME = "Media Reactive Engine"
if os.name == "nt":
    APP_DATA_ROOT = Path(os.environ.get("LOCALAPPDATA", Path.home())) / APP_NAME
    LEGACY_APP_DATA_ROOT = Path(os.environ.get("LOCALAPPDATA", Path.home())) / LEGACY_APP_NAME
else:
    APP_DATA_ROOT = Path.home() / ".magic-mic"
    LEGACY_APP_DATA_ROOT = Path.home() / ".media-reactive-engine"
CONFIG_FILE = APP_DATA_ROOT / "config.json"
LEGACY_CONFIG_FILE = LEGACY_APP_DATA_ROOT / "config.json"
...
def ensure_app_folders():
    APP_DATA_ROOT.mkdir(parents=True, exist_ok=True); DEFAULT_VISUAL_ROOT.mkdir(parents=True, exist_ok=True); DEFAULT_AUDIO_ROOT.mkdir(parents=True, exist_ok=True)
...
def save_config():
    ...
    CONFIG_FILE.write_text(json.dumps({"collection_root": str(MEDIA_ROOT), "visual_root": str(VISUAL_ROOT), "audio_root": str(AUDIO_ROOT)}, indent=2), encoding="utf-8")

def choose_folder(initial_dir=None, title="Choose Media Folder"):
...
def empty_deck():
    return {"source_folder":None,"media":None,"playing":True,"opacity":1.0,"opacity_reactive":False,"scale":1.0,"scale_reactive":False,"speed":1.0,"speed_reactive":False,"cut_token":None,"cut_position":None}

VISUAL_STATE = {
    "decks":{"a":empty_deck(),"b":empty_deck()},"crossfader":0.0,"blackout":False,"audio_reactive":True,"reactivity":0.55,
    "master_opacity":1.0,"master_brightness":1.0,"master_contrast":1.0,"master_saturation":1.0,"master_hue":0.0,"master_blur":0.0,
    "master_brightness_reactive":False,"master_brightness_amount":0.0,"master_contrast_reactive":False,"master_contrast_amount":0.0,
    "master_blur_reactive":False,"master_blur_amount":0.0,"fx_glitch":0.0,"fx_glitch_reactive":False,"fx_glitch_amount":0.0,
    "fx_rgb":0.0,"fx_rgb_reactive":False,"fx_rgb_amount":0.0,"fx_invert":0.0,"fx_invert_reactive":False,"fx_invert_amount":0.0,
    "global_master":1.0,"common_intensity":0.0,"common_filter":0.0,"common_pulse":0.0,"common_strobe":0.0,
    "text_enabled":False,"text_content":"","text_size":64.0,"text_opacity":1.0
}
...
def merge_visual_state(payload):
    keys=tuple(k for k in VISUAL_STATE.keys() if k != "decks")
    for key in keys:
...
@app.get("/regia")
def regia(): return render_template("regia.html")
@app.get("/output")
def output(): return render_template("output.html")
@app.get("/dj")
def dj(): return render_template("dj.html")
@app.get("/api/library")
def library(): return jsonify(scan_media_library(AUDIO_ROOT,source="audio"))
@app.get("/api/library/visual")
def visual_library(): return jsonify(scan_media_library(VISUAL_ROOT,source="visual"))
@app.get("/api/library/audio")
def audio_library(): return jsonify(scan_media_library(AUDIO_ROOT,source="audio"))
@app.get("/api/config")
def config(): return jsonify({"app_name":APP_NAME,"media_root":str(MEDIA_ROOT),"visual_root":str(VISUAL_ROOT),"audio_root":str(AUDIO_ROOT),"app_data_root":str(APP_DATA_ROOT),"exists":MEDIA_ROOT.exists(),"saved":CONFIG_FILE.exists()})
...
def select_media_root():
    ...
    if not selected:return jsonify({"cancelled":True,"media_root":str(MEDIA_ROOT)})
    MEDIA_ROOT=selected;VISUAL_ROOT=MEDIA_ROOT/"video";AUDIO_ROOT=MEDIA_ROOT/"audio";VISUAL_ROOT.mkdir(parents=True,exist_ok=True);AUDIO_ROOT.mkdir(parents=True,exist_ok=True);save_config()
...
def select_library_root(kind):
    ...
    save_config();scanned=scan_media_library(selected,source=kind);return jsonify({"cancelled":False,"kind":kind,"path":str(selected),"total_files":scanned.get("total_files",0),"folders":len(scanned.get("folders",[]))})
@app.get("/api/state")
...
def media_file():
    ...
    if not target or not target.is_file():return jsonify({"error":"Media not found"}),404
