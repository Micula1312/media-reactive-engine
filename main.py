...
def select_media_root():
    ...
    MEDIA_ROOT = selected; VISUAL_ROOT = MEDIA_ROOT / "video"; AUDIO_ROOT = MEDIA_ROOT / "audio"
    VISUAL_ROOT.mkdir(parents=True, exist_ok=True); AUDIO_ROOT.mkdir(parents=True, exist_ok=True); save_config()
...
def select_library_root(kind):
    ...
    save_config(); scanned = scan_media_library(selected, source=kind)
...
@app.get("/media")
def media_file():
    relative_path = request.args.get("path", ""); source = request.args.get("source")
